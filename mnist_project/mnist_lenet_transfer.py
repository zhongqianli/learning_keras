...
def mnist_lenet_transfer(input_shape, classes):
    model = keras.models.load_model('./model/mnist_lenet_data_generator.h5')

    print('transfer 0-1-2 pretrained model to 5-9 classification...')
    output_layer = model.get_layer(name='max_pooling2d_2')
    base_model = Model(inputs=model.input, outputs=output_layer.output)
...
if __name__ == '__main__':
    ...
    if len(ckpt_filelist) == 0:
        model = mnist_lenet_transfer(input_shape, num_classes)
    else:
    ...
    csvlogger = keras.callbacks.CSVLogger('{0}/log.csv'.format(log_dir), separator=',', append=False)

    history = model.fit_generator(generator=train_generator,
                        steps_per_epoch=len(train_generator),
                        epochs=epochs,
                        verbose=1,
                        callbacks=[tensorboard, checkpointer, csvlogger],
